...
if __name__ == '__main__':
   ...
   def lookup_columns(df, vocab):
    ...
    for col in cat_dfs:
        cat_df = cat_dfs[col]
        #joinCol = '%sToJoin' % col
        #cat_indexed = train_df.join(cat_df.hint("broadcast"), train_df[col] == cat_df[joinCol], 'left_outer').drop(joinCol)
        cat_indexed = train_df.join(cat_df.hint("broadcast"), [col], 'left_outer') #.drop(joinCol)
        train_df = cat_indexed.drop(col).withColumnRenamed('row_number', col)
    ...
    train_df.write.mode("overwrite").parquet("hdfs:///user/tgraves/horovodtrainoutnoudf")
    test_df.write.mode("overwrite").parquet("hdfs:///user/tgraves/horovodtestoutnoudf")
